...
$(function () {
    ...
    fetch(url, settings)
        ...
        .then(res => {
            ...
            setFileContent("sample4-simple-json");
    ...
    cy = cytoscape({
        ...
        ready: function () {
            ...
            this.nodes().forEach((node) => {
                ...
                node.css("width", node.data().id.startsWith("r") || node.data().id.startsWith("p") || node.data().id.startsWith("b") ? 15 : size);
                node.css("height", /*node.data().height*/  node.data().id.startsWith("r") || node.data().id.startsWith("p") || node.data().id.startsWith("b") ? 15:  size);
                node.css("font-size", 5);
                node.data("label", node.data().id);
                if( node.data().id.startsWith( "r"))
                node.css( "background-color", "red");
                if( node.data().id.startsWith( "b"))
                    node.css( "background-color", "green");
                if( node.data().id.startsWith( "p"))
...
var COSEBilkentLayout = Backbone.View.extend({
    ...
    render: function () {
        ...
        $("#save-layout4").click(function (evt) {
            verticalSpacing = self.currentLayoutProperties.verticalSpacing = Number(document.getElementById("vertical-space").value);
            horizontalSpacing = self.currentLayoutProperties.horizontalSpacing = Number(document.getElementById("horizontal-space").value);
