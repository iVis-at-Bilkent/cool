...
// POST /layout/:format?edges=true 
// POST /layout/:format?clusters=true
app.post('/presetlayout/:format', (req, res) => {
    options.animate = false;
    let size = 30;
    console.log( "preset layout is started");
    ...
    cy.filter((element, i) => {
        ...
        if (ele.isNode()) {
            if (req.params.format === "json") {
                let obj = {};
                console.log( "Ele data before layout : " + ele.position().x);
                obj["position"] = { x: parseInt(ele.position().x), y: parseInt(ele.position().y)};
                obj["data"] = { width: 30, height:30, clusterID: ele.data().clusterID, parent: ele.data().parent, label : (("n") + ele.data().id ) };
                ret[ele.data().id] = obj;
            }
        }
